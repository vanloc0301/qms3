       ...
       // QMS3.CfCardPC.CfCardPC cardrelated = new QMS.CfCardPC.CfCardPC();
        ...
        private void treeView1_AfterSelect_1(object sender, TreeViewEventArgs e)
        {
            ...
            switch (treeView1.SelectedNode.ToString())
            {
                ...
                break;
                case "TreeNode: 在运车辆查询":      MainTab.SelectTab(7);
                break;
                case "TreeNode: 车辆信息查询":      MainTab.SelectTab(23);
                break;
                ...
                case "TreeNode: 垃圾楼管理":
                {
                    ...
                    string strSQL = "SELECT s.ID AS 'ID' ,s.StationID AS '站号', s.Name AS '站名', s.Class AS '所属班号',e.Employee AS '当前值班人' FROM [dbo.Station] as s INNER JOIN [dbo.Employee] as e ON s.StationID = e.StationID and s.EmployeeNum = e.num";
                    ...
                    strSQL = "SELECT * FROM [dbo.Employee]";
                    strTable = " [db_rfidtest].[rfidtest].[dbo.Employee]";
                    try
                    {
                        dtEmployee = boperate.getds(strSQL, strTable).Tables[0];
                    }
                    catch { }
  ...
  create table res(staname  varchar(100),sumbox int,weight2 float,weight3 float,weight4 float,weight5 float,weight6 float,weight7 float,weight8 float,weight9 float,weight10 float,weight11 float,weight12 float,weight13 float,weight14 float,weight15 float,sumweight float,sumboxtail float,dateid varchar(100));
        ...
        private void backgroundWorkerYearOp_DoWork(object sender, DoWorkEventArgs e)//生成年表
        {
            ...
            for (int cur_mon = 1; cur_mon <= 12; cur_mon++)//2替换cur_month_days。按站分组，每次生成新表的一行
            {
                ...
                #region 两个SQL，一个查全年，一个查每个月，结果放在两个表中，为了子母报表
...
drop table tempTable;";
        ...
        private void button16_Click(object sender, EventArgs e)//车查询
        {
            ...
            if (comboBox13.SelectedIndex > 0)
                strSQL += " AND g.EndStationID = " + comboBox13.SelectedIndex;
...
#region 垃圾转运中心自动服务
        ...
        private void button25_Click(object sender, EventArgs e)
        {
           ...
           "WHERE  g.[EndTime] > '" + dateTimePicker7.Value.ToString("yy-MM-dd") + ",00:00' AND g.[EndTime] < '" + dateTimePicker7.Value.ToString("yy-MM-dd") + ",23:59'";
            if (comboBox10.SelectedIndex > 0)
                strSQL += " AND g.EndStationID = " + comboBox10.SelectedIndex;  
            string strTable = " [db_rfidtest].[rfidtest].[dbo.goods]";
            try
            {
                ds = boperate.getds(strSQL, strTable);
        ...
        private void dataGridView5_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (int.Parse(row["StationID"].ToString()) == int.Parse(dataGridView5.Rows[e.RowIndex].Cells[1].Value.ToString()))
                    {
                        EmployeeClass em = new EmployeeClass();
                        em.Id = int.Parse(row["ID"].ToString());
        ...
        private void LoadEmployee(int RowIndex)
        {
            ...
            foreach (DataRow row in dtEmployee.Rows)
            {
                if (int.Parse(row["StationID"].ToString()) == int.Parse(dataGridView5.Rows[RowIndex].Cells[1].Value.ToString()))
                {
                    EmployeeClass em = new EmployeeClass();
                    em.Id = int.Parse(row["ID"].ToString());
                    em.StationID = int.Parse(row["StationID"].ToString());
        ...
        private void button22_Click(object sender, EventArgs e)
        {
            ...
            string strSQL = "SELECT DISTINCT s1.[Name] AS '起始站点' , s2.Name AS '到达站点'," +
                ...
                " FROM  [db_rfidtest].[rfidtest].[dbo.Goods] AS g INNER JOIN  [db_rfidtest].[rfidtest].[dbo.Station] AS s1 ON   " +
                "g.[StartStationID] = s1.[StationID] INNER JOIN [dbo.Station] AS s2 ON g.EndStationID = s2.StationID  " +
                "WHERE  g.[EndTime] > '" + systime + ",00:00' AND g.[EndTime] < '" + systime + ",23:59' AND  g.[State] <0";
            string strTable = " [db_rfidtest].[rfidtest].[dbo.goods]";

            if (comboBox11.SelectedIndex > 0)
                strSQL += " AND g.EndStationID = " + comboBox11.SelectedIndex;
        ...
        private void button21_Click(object sender, EventArgs e)
        {
            if (txtEmp.Tag == null)
            {
                MessageBox.Show("请选择要修改的值班人");
                return;
            }

            string sql = "UPDATE [dbo.Employee] SET Employee = '" + txtEmp.Text + "' WHERE ID = " + txtEmp.Tag.ToString();

            boperate.getcom(sql);

            LoadEmployee(dataGridView5.SelectedCells[0].RowIndex);
        }

        private void button60_Click(object sender, EventArgs e)
        {
            if (dgvEmp.SelectedCells.Count <= 0)
                return;
            List<EmployeeClass> list = (List<EmployeeClass>)dgvEmp.DataSource;
            string sql = "DELETE FROM [dbo.Employee] WHERE ";
            foreach (DataGridViewCell item in dgvEmp.SelectedCells)
            {
                sql += " ID = " + list[item.RowIndex].Id + " or";    
            }
        ...
        private void button67_Click(object sender, EventArgs e)
        {
            ...
            List<EmployeeClass> list = (List<EmployeeClass>)dgvEmp.DataSource;
            string sql = "UPDATE [dbo.Station] SET EmployeeNum = " + list[dgvEmp.SelectedCells[0].RowIndex].Num + " WHERE stationID=" + list[dgvEmp.SelectedCells[0].RowIndex].StationID;
        ...
        private void dgvEmp_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex < 0)
                return;
            txtEmp.Tag = list[e.RowIndex].Id;
            txtEmp.Text = list[e.RowIndex].Employee;
        }
        ...
        private void button63_Click(object sender, EventArgs e)
        {
            ...
            if (dgvEmp.DataSource == null)
                return;
            List<EmployeeClass> list = (List<EmployeeClass>)dgvEmp.DataSource;
            foreach (EmployeeClass emp in list)
            {
                if (emp.Num > num)
                    num = emp.Num;
            }
