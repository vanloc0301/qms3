        ...
        public void updateUIlog(Task A_card)//更新UI 书写log
        {
            ...
            try
            {
                temp =
                    ...
                    + A_card.StartTime + " \t"
                    + A_card.EndTime + " \t"
                    + TransCenter.StationName[A_card.StartSpot - 31];
            }
            catch
            {
            ...
            {
                string k = listBox1.Items[i].ToString();
                //MessageBox.Show(k.Substring(0,56));
                //MessageBox.Show(temp2);
                try
                {
                    if (k.Substring(0, 58) == (temp2))
                        //MessageBox.Show("相等");
                        return;
                }
                catch (Exception)
                ...
                if (A_card.log == ("数据转换时出错"))
                    //MessageBox.Show("数据转换时出错");
                    return;
            }
