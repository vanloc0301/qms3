...
namespace InfoView.Classes
{
    public class visifire
    {
        ...
        public int titleSize = 72;
        public int labelSize = 46;
        public string doc;
        public string title;
        ...
        private string genXml()
        {
            if (len <= 0)
                return "";
            string xml = "";
            xml += "<vc:Chart xmlns:vc=\"clr-namespace:Visifire.Charts;assembly=SLVisifire.Charts\" Width=\"" + (this.width).ToString() + "\" Height=\"" + (this.height).ToString() + "\" BorderThickness=\"0\" Theme=\"Theme2\" " + this.View3d + " ToolBarEnabled=\"False\">";
            xml += "<vc:Chart.Background>";
            xml += "<SolidColorBrush Color=\"#256acf\"/>";
            xml += "</vc:Chart.Background>";
            xml += "<vc:Chart.Titles>";
            xml += "<vc:Title Text=\"" + this.title + "\" FontSize=\""+titleSize+"\"/>";
            xml += "</vc:Chart.Titles>";
            xml += "<vc:Chart.AxesX>";
            xml += "<vc:Axis Title=\"" + this.xtitle + " \" FontSize=\""+titleSize+"\" >";
            xml += "<vc:Axis.AxisLabels >";
            xml += "<vc:AxisLabels FontSize = \""+labelSize+"\" />";
            xml += "</vc:Axis.AxisLabels>";
            xml += "</vc:Axis>";
            xml += "</vc:Chart.AxesX>";
            xml += "<vc:Chart.AxesY>";
            xml += "<vc:Axis Title=\"" + this.ytitle + "\" FontSize=\""+titleSize+"\">   ";
            xml += "<vc:Axis.AxisLabels >";
            xml += "<vc:AxisLabels FontSize = \""+labelSize+"\" />";
            xml += "</vc:Axis.AxisLabels>";
            xml += "</vc:Axis>";
            ...
            xml += "<vc:DataSeries.DataPoints>";
                for (int i = 0; i < this.len; i++)
                    xml += "<vc:DataPoint AxisXLabel=\"" + this.column[i] + "\" YValue=\"" + this.data[i] + "\" LabelFontSize=\"20\" />";
        ...
        {
            string xml = genXml();
            string size = "var vChart = new Visifire('SL.Visifire.Charts.xap', \"MyChart\", " + (this.width) + ", " + (this.height) + ");"
                           + "var chartXml = '";
        ...
        public Uri displayChart()
        {
            ...
            Uri url = new Uri(str + "chart/" + this.title + ".htm", UriKind.RelativeOrAbsolute);
            return url;
        }
        public Uri refresh()
        {
